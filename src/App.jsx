...
const C = {
  ...
  jump: "#a3e635", text: "#f2f6fb", muted: "#9db1ce",
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "flex-start", gap: 12, marginBottom: 20 }}>
          <div style={{
            ...
            fontSize: 16, fontWeight: 700, color: C.men,
          }}>
            {displayName.charAt(0)}
          </div>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 20, fontWeight: 700, color: C.text }}>{displayName}</div>
            {player && (
        ...
        {loading && (
          <div style={{ textAlign: "center", padding: "40px 0", color: C.muted }}>
            ...
            <div style={{ fontSize: 15 }}>読み込み中...</div>
          </div>
        )}

        {!loading && error && (
          <div style={{ background: C.negative + "22", border: `1px solid ${C.negative}44`, borderRadius: 10, padding: 14, color: C.negative, fontSize: 15 }}>
            {error}
          </div>
        )}

        {!loading && !error && compIds.length === 0 && (
          <div style={{ textAlign: "center", padding: "32px 0", color: C.muted, fontSize: 15 }}>
            過去の大会記録がありません
          </div>
        )}
          ...
          return (
            <div key={cid} style={{
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                ...
                {isLatest && (
                  <span style={{ fontSize: 12, background: C.accent + "22", color: C.accent, border: `1px solid ${C.accent}44`, borderRadius: 10, padding: "1px 7px" }}>
                    最新
                  </span>
                )}
              </div>
                ...
                {EVENTS.map(ev => {
                  ...
                  return (
                    <div key={ev} style={{
                      ...
                      <div style={{ fontSize: 12, fontWeight: 700, color: hasScore ? ECFG[ev].color : C.muted, marginBottom: 4 }}>
                        {ECFG[ev].label}
                      </div>
                      <div style={{ fontSize: 14, fontFamily: "monospace", color: hasScore ? C.text : C.muted, fontWeight: hasScore ? 700 : 400, lineHeight: 1.4 }}>
                        {formatted || "—"}
                      </div>
                    </div>
                  );
        ...
        {player?.en_names && !loading && (
          <div style={{ marginTop: 10, fontSize: 12, color: C.muted + "99", textAlign: "center" }}>
    ...
    <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
      {[{ key: "men", label: "👨 男子", color: C.men }, { key: "women", label: "👩 女子", color: C.women }].map(g => (
        <button key={g.key} onClick={() => onChange(g.key)} style={{
          ...
          fontSize: 16, fontWeight: gender === g.key ? 700 : 400,
...
function SectionHeader({ title, color, right }) {
  return (
    <div style={{
      fontSize: 14, fontWeight: 700, color: color || C.muted,
      background: C.bg, borderBottom: `1px solid ${C.border}`,
      padding: "10px 14px", display: "flex", justifyContent: "space-between", alignItems: "center",
    }}>
      <span>{title}</span>
      {right && <span style={{ fontSize: 12, color: C.muted }}>{right}</span>}
    </div>
  );
}
...
function MiniProgress({ filled, total, color }) {
  ...
  return (
    <span style={{ fontFamily: "monospace", fontSize: 13, color: done ? C.positive : color }}>
      {filled}/{total}{done ? " ✅" : ""}
    </span>
  );
}
  ...
  return (
    <input
      ...
      style={{
        ...
        color: C.text, fontSize: 16, padding: "8px 10px", outline: "none",
      ...
      >
        <div onClick={e => e.stopPropagation()} style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
            <div>
              <div style={{ fontSize: 18, fontWeight: 700, color: ecfg.color }}>{ecfg.label}</div>
              <div style={{ fontSize: 14, color: C.muted }}>{school}　{cfg.topN}人どり</div>
            </div>
            <div style={{ marginLeft: "auto", textAlign: "right" }}>
              <div style={{ fontSize: 13, color: C.muted }}>チーム合計</div>
              <div style={{ fontSize: 21, fontWeight: 900, fontFamily: "monospace", color: C.accent }}>
                {result.totalPts !== null ? `${result.totalPts.toFixed(1)}pt` : "—"}
              </div>
              {result.totalScore !== null && (
                <div style={{ fontSize: 13, color: ecfg.color }}>
            ...
            return (
              <div key={i} style={{
                ...
                <div style={{
                  ...
                  display: "flex", alignItems: "center", justifyContent: "center",
                  fontSize: 13, fontWeight: 700, color: isAdopted ? C.bg : C.muted,
                }}>{i + 1}</div>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  >
                    {sk.name || `選手${i + 1}`}
                    {sk.name && (
                      <span style={{ fontSize: 12, color: C.muted, opacity: 0.7 }}>📋</span>
                    )}
                  </div>
  ...
  return (
    <div style={{ display: "grid", gap: 14 }}>
      <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: 16 }}>
        <div style={{ fontSize: 15, fontWeight: 700, color: cfg.color, marginBottom: 12 }}>
      ...
      <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: 16 }}>
        <div style={{ fontSize: 15, fontWeight: 700, color: C.text, marginBottom: 12 }}>📋 集計設定</div>
      ...
      <button
        ...
        style={{
          ...
          fontSize: 16, fontWeight: 700, padding: "13px",
      ...
      >
        ...
        <div style={{ fontSize: 15, fontWeight: 700, color: C.accent, marginBottom: 8 }}>🔄 「実際」だけリセット</div>
        <div style={{ fontSize: 14, color: C.muted, marginBottom: 12 }}>全種目・全校の「実際」の記録だけを消去します。「想定」はそのまま残るので、朝一やシミュレーションの前に使えます。</div>
        <button
          ...
          style={{
            ...
            fontSize: 15, fontWeight: 700, padding: "10px 20px", cursor: "pointer", width: "100%",
          }}
        >
      ...
      <div style={{ background: C.surface, border: `1px solid ${C.negative}33`, borderRadius: 12, padding: 16 }}>
        <div style={{ fontSize: 15, fontWeight: 700, color: C.negative, marginBottom: 8 }}>⚠️ データリセット</div>
        <div style={{ fontSize: 14, color: C.muted, marginBottom: 12 }}>全ての入力データ・設定を初期化します。この操作は元に戻せません。</div>
        <button onClick={onReset} style={{ background: C.negative + "22", border: `1px solid ${C.negative}66`, borderRadius: 8, color: C.negative, fontSize: 15, fontWeight: 700, padding: "10px 20px", cursor: "pointer", width: "100%" }}>
          🗑 全データをリセット
        </button>
      </div>
    </div>
    ...
    <div>
        ...
        {EVENTS.map(e => {
          ...
          return (
            <button key={e} onClick={() => setEvent(e)} style={{
              ...
              fontSize: 15, fontWeight: event === e ? 700 : 400,
              padding: "10px 6px", cursor: "pointer", textAlign: "center",
            }}>
              <div style={{ marginBottom: 4, fontSize: 14 }}>{c.label}</div>
        ...
        {SCHOOLS.map(s => {
          ...
          return (
            <button key={s} onClick={() => setSchool(s)} style={{
              ...
              <div>{s}</div>
              <div style={{ fontSize: 12, color: school === s ? "#ffffffaa" : done ? C.positive : C.muted, fontFamily: "monospace" }}>
                {f}/{skrs.length}{done ? " ✅" : ""}
              </div>
            </button>
          );
      ...
      <div style={{ background: C.surface, border: `1px solid ${ecfg.color}33`, borderRadius: 10, padding: "10px 14px", marginBottom: 14 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
          <span style={{ fontSize: 14, color: ecfg.color, fontWeight: 700 }}>{ecfg.label}　{school}</span>
        ...
        {event === "jump" && <div style={{ fontSize: 12, color: C.jump, marginTop: 6 }}>※ Jハンデ -{cfg.handicap}m を引いて換算点を計算します</div>}
      </div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
              ...
              {sk.name && (
                <button
                  ...
                  style={{ background: C.surface2, border: `1px solid ${C.border}`, borderRadius: 8, color: C.muted, fontSize: 13, padding: "4px 10px", cursor: "pointer", flexShrink: 0, display: "flex", alignItems: "center", gap: 4 }}
                >
                  📋 実績
                </button>
              )}
              {hasActual && (
                <span style={{ fontSize: 12, background: C.positive + "22", color: C.positive, border: `1px solid ${C.positive}44`, borderRadius: 10, padding: "2px 8px" }}>✅</span>
              )}
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
              <div>
              ...
              <div>
                <div style={{ fontSize: 12, color: C.muted, marginBottom: 4 }}>換算点</div>
                <div style={{ background: pts !== null ? C.accent + "11" : C.bg, border: `1px solid ${pts !== null ? C.accent + "44" : C.border}`, borderRadius: 6, padding: "8px 10px", fontFamily: "monospace", fontSize: 16, color: pts !== null ? C.accent : C.muted, fontWeight: pts !== null ? 700 : 400, textAlign: "center" }}>
              ...
              return (
                <tr key={r.school} style={{ borderBottom: `1px solid ${C.border}` }}>
                  ...
                  {EVENTS.map(e => {
                    ...
                    return (
                      <td key={e} style={{ padding: "6px 4px", textAlign: "center", fontFamily: "monospace", fontSize: 13, color: d === null ? C.muted : ECFG[e].color, whiteSpace: "nowrap" }}>
              ...
              return (
                ...
                >
                  ...
                  {EVENTS.map(e => {
                    const effPin = e === "jump" ? Math.max(0, parseFloat(cfg.pin[e]) - parseFloat(cfg.handicap)) : parseFloat(cfg.pin[e]);
                    return (
                      <td key={e} style={{ padding: "6px 4px", textAlign: "center", fontFamily: "monospace", fontSize: 13, color: dPlan === null ? C.muted : ECFG[e].color, whiteSpace: "nowrap" }}>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
              <thead>
                <tr style={{ background: C.bg, borderBottom: `1px solid ${C.border}` }}>
                  ...
                  <th style={{ padding: "6px 8px", textAlign: "center" }}><div style={{ color: ecfg.color }}>得点差（{ecfg.unit}）</div><div style={{ color: C.accent, fontSize: 12 }}>換算点差（pt）</div></th>
                  <th style={{ padding: "6px 8px", textAlign: "center", color: C.muted, fontSize: 12 }}><div>慶應</div><div>相手</div></th>
                </tr>
              </thead>
              <tbody>
                {others.map(r => {
                  ...
                  return (
                    <tr key={r.school} style={{ borderBottom: `1px solid ${C.border}`, cursor: "pointer" }}
                      ...
                      <td style={{ padding: "6px 8px", textAlign: "center" }}>
                        <div style={{ fontFamily: "monospace", fontSize: 15, fontWeight: 700, color: diffColor(sDiff) }}>{sDiff !== null ? `${sDiff >= 0 ? "+" : ""}${sDiff}${ecfg.unit}` : "—"}</div>
                        <div style={{ fontFamily: "monospace", fontSize: 13, color: diffColor(ptDiff), marginTop: 2 }}>{signStr(ptDiff, "pt")}</div>
                      </td>
                ...
                >
                  ...
                  <td style={{ padding: "6px 8px", textAlign: "center", fontSize: 11, color: C.muted, lineHeight: 1.7 }}>
                    {dPts === null ? "実績 vs 想定" : EVENTS.filter(ev2 => ev2 !== e).map(ev2 => {
                      ...
                      return (
                        <div key={ev2} style={{ color: ecfg2.color }}>
                          {EVENT_SHORT_LABEL[ev2]}:{amount >= 0 ? "+" : ""}{amount}{ecfg2.unit}
                        </div>
                      );
                    })}
                  </td>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
              <thead>
                <tr style={{ background: C.bg, borderBottom: `1px solid ${C.border}` }}>
                  ...
                  <th style={{ padding: "6px 8px", textAlign: "center", color: C.muted, fontSize: 12 }}>入力</th>
                </tr>
              </thead>
              <tbody>
                  ...
                  return (
                    <tr key={school} onClick={() => setPopup({ school, event: e })} style={{ borderBottom: `1px solid ${C.border}`, background: school === "慶應" ? C.keio + "11" : "transparent", cursor: "pointer" }}>
                      <td style={{ padding: "10px 10px", fontWeight: school === "慶應" ? 700 : 400, color: school === "慶應" ? C.keio : C.text }}>
                        {school} <span style={{ fontSize: 12, color: C.muted }}>▶</span>
                      </td>
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      <div style={{
        ...
        padding: "10px 14px", fontSize: 13, color: C.accent, display: "flex", alignItems: "center", gap: 8,
      }}>
    ...
    <div>
      <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 10, padding: 12, marginBottom: 14 }}>
        <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
          {[{ key: "A", title: "Aモード", sub: "入力済みのみ" }, { key: "B", title: "Bモード", sub: "実際＋想定混在" }].map(m => (
            <button key={m.key} onClick={() => setMode(m.key)} style={{ flex: 1, background: mode === m.key ? C.accent + "22" : C.surface2, border: `1px solid ${mode === m.key ? C.accent : C.border}`, borderRadius: 8, color: mode === m.key ? C.accent : C.muted, fontSize: 15, fontWeight: mode === m.key ? 700 : 400, padding: "8px 10px", cursor: "pointer" }}>
              <div>{m.title}</div><div style={{ fontSize: 12, marginTop: 2 }}>{m.sub}</div>
            </button>
          ))}
        </div>
        <div style={{ fontSize: 12, color: C.muted, marginBottom: 6 }}>入力進捗</div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
          {EVENTS.map(e => {
            const ecfg = ECFG[e];
            const done = completedEvents.includes(e);
            return (
              <div key={e}>
                <div style={{ fontSize: 12, color: done ? C.positive : ecfg.color, marginBottom: 4 }}>{ecfg.label}{done ? " ✅" : ""}</div>
                {SCHOOLS.map(s => {
                  ...
                  return (
                    <div key={s} style={{ display: "flex", alignItems: "center", gap: 4, marginBottom: 3 }}>
                      <div style={{ fontSize: 12, color: s === "慶應" ? C.keio : C.muted, width: 38, flexShrink: 0 }}>{s}</div>
                      ...
                      <div style={{ fontSize: 12, color: sdone ? C.positive : C.muted, fontFamily: "monospace", width: 22, textAlign: "right" }}>{f}/{t}</div>
                    </div>
                  );
                })}
              </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(72px, 1fr))", gap: 6, marginBottom: 14 }}>
        ...
        {[{ key: "diff", label: "📉 差分分析" }, { key: "breakdown", label: "📋 種目別内訳" }, { key: "planned", label: "📌 想定" }].map(v => (
          <button key={v.key} onClick={() => setView(v.key)} style={{ flex: 1, background: view === v.key ? C.surface : C.surface2, border: `1px solid ${view === v.key ? C.accent : C.border}`, borderRadius: 8, color: view === v.key ? C.accent : C.muted, fontSize: 15, fontWeight: view === v.key ? 700 : 400, padding: "8px", cursor: "pointer" }}>{v.label}</button>
        ))}
      </div>
        ...
        return (
          <div key={`${sk.gender}-${sk.srcIdx}`} style={{
            ...
            <div style={{
              ...
              border: `1px solid ${isAdopted ? C.accent : C.border}`,
              display: "flex", alignItems: "center", justifyContent: "center",
              fontSize: 14, color: isAdopted ? C.bg : (sk.gender === "men" ? C.men : C.women),
            }}>{sk.gender === "men" ? "👨" : "👩"}</div>
    ...
    <>
      ...
      >
        <div onClick={e => e.stopPropagation()} style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
            <div>
              <div style={{ fontSize: 18, fontWeight: 700, color: ecfg.color }}>{ecfg.label}　男女混合</div>
              <div style={{ fontSize: 14, color: C.muted }}>{school}　{COMBINED_TOP_N}人どり（{result.total}人中）</div>
            </div>
            <div style={{ marginLeft: "auto", textAlign: "right" }}>
              <div style={{ fontSize: 13, color: C.muted }}>チーム合計</div>
              <div style={{ fontSize: 21, fontWeight: 900, fontFamily: "monospace", color: C.accent }}>
...
function CombinedDiffDetailPopup({ event, dPts, rosterResult, rosterLabel, config, onClose }) {
  const ecfg = ECFG[event];
    ...
    >
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
          <div>
            <div style={{ fontSize: 18, fontWeight: 700, color: ecfg.color }}>{ecfg.icon} {ecfg.label}　男女混合</div>
            <div style={{ fontSize: 14, color: C.muted }}>{rosterLabel}　対 慶應</div>
          </div>
        ...
        <div style={{ textAlign: "center", marginBottom: 14 }}>
          <div style={{ fontSize: 12, color: C.muted, marginBottom: 2 }}>慶應との換算点差</div>
          <div style={{ fontSize: 29, fontWeight: 900, fontFamily: "monospace", color: diffColor(dPts) }}>{signStr(dPts, "pt")}</div>
        </div>
        ...
        {dPts !== null && (
          <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
            <div style={{ flex: 1, background: C.men + "11", border: `1px solid ${C.men}44`, borderRadius: 10, padding: "10px 12px", textAlign: "center" }}>
              <div style={{ fontSize: 12, color: C.men, marginBottom: 4 }}>👨 男子の{ecfg.label}なら</div>
              <div style={{ fontSize: 17, fontWeight: 700, fontFamily: "monospace", color: C.men }}>
            ...
            <div style={{ flex: 1, background: C.women + "11", border: `1px solid ${C.women}44`, borderRadius: 10, padding: "10px 12px", textAlign: "center" }}>
              <div style={{ fontSize: 12, color: C.women, marginBottom: 4 }}>👩 女子の{ecfg.label}なら</div>
              <div style={{ fontSize: 17, fontWeight: 700, fontFamily: "monospace", color: C.women }}>
...
function eventEffPin(event, gender, config) {
  return event === "jump"
    ? Math.max(0, parseFloat(config[gender].pin.jump) - parseFloat(config[gender].handicap))
    : parseFloat(config[gender].pin[event]);
}

function InlineGenderConv({ dEv, event, config }) {
  if (dEv === null) return <span style={{ color: C.muted, fontSize: 13 }}>—</span>;
  const sign = dEv >= 0 ? "+" : "-";
  const menU   = ptToUnit(dEv, eventEffPin(event, "men", config));
  const womenU = ptToUnit(dEv, eventEffPin(event, "women", config));
  return (
    <div style={{ lineHeight: 1.35 }}>
      <div style={{ fontSize: 13, color: C.men }}>男{sign}{menU}</div>
      <div style={{ fontSize: 13, color: C.women }}>女{sign}{womenU}</div>
    </div>
  );
}
    ...
    <div style={{ display: "grid", gap: 12 }}>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
              ...
              <tbody>
                {others.map(r => {
                  ...
                  const rEv = r.result.ev[e];
                  const ptDiff = kEv.totalPts !== null && rEv.totalPts !== null ? kEv.totalPts - rEv.totalPts : null;
                  const sign = ptDiff !== null && ptDiff >= 0 ? "+" : "-";
                  return (
                    <tr key={r.school} style={{ borderBottom: `1px solid ${C.border}`, cursor: "pointer" }}
                  ...
                  <td style={{ padding: "6px 3px", textAlign: "center", fontFamily: "monospace", fontSize: 13, color: C.men, whiteSpace: "nowrap" }}>
                    {dPts === null ? "—" : (
                      <>
                        ...
                        {EVENTS.filter(ev2 => ev2 !== e).map(ev2 => (
                          <div key={ev2} style={{ fontSize: 11, color: ECFG[ev2].color, marginTop: 1 }}>
                            {EVENT_SHORT_LABEL[ev2]}:{dPts >= 0 ? "+" : "-"}{ptToUnit(dPts, eventEffPin(ev2, "men", config))}{ECFG[ev2].unit}
                          </div>
                        ))}
                      </>
                    )}
                  ...
                  <td style={{ padding: "6px 3px", textAlign: "center", fontFamily: "monospace", fontSize: 13, color: C.women, whiteSpace: "nowrap" }}>
                    {dPts === null ? "—" : (
                      <>
                        ...
                        {EVENTS.filter(ev2 => ev2 !== e).map(ev2 => (
                          <div key={ev2} style={{ fontSize: 11, color: ECFG[ev2].color, marginTop: 1 }}>
                            {EVENT_SHORT_LABEL[ev2]}:{dPts >= 0 ? "+" : "-"}{ptToUnit(dPts, eventEffPin(ev2, "women", config))}{ECFG[ev2].unit}
                          </div>
                        ))}
                      </>
                    )}
      ...
      {diffPopup && (
        <CombinedDiffDetailPopup
          ...
          dPts={diffPopup.dPts}
          rosterResult={diffPopup.rosterResult}
  ...
  return (
    <div>
      <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 10, padding: 12, marginBottom: 14 }}>
        ...
        <div style={{ fontSize: 12, color: C.muted, marginBottom: 6 }}>入力進捗（男女混合・種目ごとに上位{COMBINED_TOP_N}人採用）</div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
          {EVENTS.map(e => {
            const ecfg = ECFG[e];
            const done = completedEvents.includes(e);
  ...
  return (
    <div>
      <div style={{ marginBottom: 12 }}>
        <button onClick={() => setSettingsOpen(v => !v)} style={{ width: "100%", background: settingsOpen?"#0d1e3a":C.surface, border: `1px solid ${settingsOpen?C.slalom:C.border}`, borderRadius: settingsOpen?"10px 10px 0 0":10, color: settingsOpen?C.slalom:C.muted, fontSize: 14, fontWeight: 700, padding: "10px 14px", cursor: "pointer", display: "flex", alignItems: "center", gap: 8 }}>
          <span>⚙️ 連盟サイトのURL設定</span>
          <span style={{ marginLeft: "auto", fontSize: 12 }}>{settingsOpen?"▲":"▼"}</span>
        </button>
        {settingsOpen && (
          <div style={{ background: C.surface, border: `1px solid #2a3a5a`, borderTop: "none", borderRadius: "0 0 10px 10px", padding: 14 }}>
            <div style={{ fontSize: 13, color: "#4a6a9a", marginBottom: 6 }}>出走リストが掲載されているページのURL</div>
            <input type="text" value={inputUrl} onChange={e => setInputUrl(e.target.value)} placeholder="https://..." style={{ width: "100%", background: "#0a1020", border: "1px solid #1e2a4a", borderRadius: 6, color: C.text, fontSize: 14, padding: "8px 10px", outline: "none", fontFamily: "monospace", boxSizing: "border-box", marginBottom: 10 }} />
            <button onClick={handleSaveUrl} style={{ width: "100%", background: "#1a3a6a", border: `1px solid ${C.slalom}`, borderRadius: 8, color: C.slalom, fontSize: 15, fontWeight: 700, padding: "10px", cursor: "pointer" }}>
              保存する
            </button>
          </div>
        )}
      ...
      {url && (
        <div style={{ background: C.surface, border: "1px solid #1e3a6e", borderRadius: 12, overflow: "hidden", marginBottom: 16 }}>
          <div style={{ background: "#0d2045", padding: "9px 14px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: C.slalom }}>🔗 連盟サイトの出走リスト</div>
            <a href={url} target="_blank" rel="noopener noreferrer" style={{ fontSize: 12, color: "#4a6a9a", textDecoration: "none", border: "1px solid #1e3060", borderRadius: 4, padding: "2px 7px" }}>開く ↗</a>
          </div>
      ...
      <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, overflow: "hidden" }}>
        <div style={{ background: "#0d1e3a", padding: "9px 14px", fontSize: 13, fontWeight: 700, color: C.jump }}>📷 配布された出走リスト（写真）</div>
        <div style={{ padding: 14 }}>
          <input ref={fileInputRef} type="file" accept="image/*" onChange={handleFileSelect} style={{ display: "none" }} />
          <button
            onClick={() => fileInputRef.current?.click()}
            disabled={uploading}
            style={{ width: "100%", background: C.jump + "22", border: `1px solid ${C.jump}66`, borderRadius: 8, color: C.jump, fontSize: 15, fontWeight: 700, padding: "12px", cursor: "pointer", marginBottom: 16 }}
          >
              ...
              {photos.map((p, i) => (
                <div key={i} style={{ position: "relative" }}>
                  ...
                  <button
                    ...
                    style={{ position: "absolute", top: 4, right: 4, background: C.negative + "cc", border: "none", borderRadius: "50%", width: 22, height: 22, color: "#fff", fontSize: 14, cursor: "pointer", lineHeight: 1 }}
                  >✕</button>
                  {editingIndex === i ? (
                    <div style={{ display: "flex", gap: 4, marginTop: 6 }}>
                      <input
                        ...
                        placeholder="タイトルを入力"
                        style={{ flex: 1, minWidth: 0, fontSize: 13, background: C.bg, border: `1px solid ${C.jump}`, borderRadius: 6, color: C.text, padding: "6px 8px", boxSizing: "border-box", outline: "none" }}
                      />
                      <button
                        onClick={() => handleTitleSave(i)}
                        style={{ flexShrink: 0, background: C.jump, border: "none", borderRadius: 6, color: C.bg, fontSize: 15, fontWeight: 900, padding: "6px 10px", cursor: "pointer" }}
                      >✓</button>
                    </div>
                  ) : (
                    <div
                      ...
                      style={{
                        marginTop: 6, fontSize: 13, fontWeight: p.title ? 700 : 400,
      ...
      {viewerPhoto && (
        ...
        >
          {viewerPhoto.title && (
            <div style={{ color: C.text, fontSize: 16, fontWeight: 700, marginBottom: 10 }}>{viewerPhoto.title}</div>
          )}
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "'Segoe UI','Helvetica Neue',sans-serif" }}>
      <div style={{ position: "sticky", top: 0, zIndex: 100, background: C.bg + "ee", backdropFilter: "blur(12px)", borderBottom: `1px solid ${C.border}` }}>
        <div style={{ maxWidth: 900, margin: "0 auto", padding: "10px 16px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            />
            <div>
              <div style={{ fontSize: 12, color: C.muted, lineHeight: 1, marginBottom: 2 }}>慶應義塾大学水上スキー部</div>
              <div style={{ fontSize: 19, fontWeight: 900, color: C.accent, fontFamily: "'Georgia',serif", letterSpacing: "-0.01em", lineHeight: 1.2 }}>KWST Dashboard</div>
            </div>
            {loading && <span style={{ fontSize: 12, color: C.muted, marginLeft: "auto" }}>読込中...</span>}
            {syncing && !loading && <span style={{ fontSize: 12, color: C.positive, marginLeft: "auto" }}>💾保存中</span>}
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: 16 }}>
        {loading && (
          <div style={{ textAlign: "center", padding: "60px 0", color: C.muted }}>
            ...
            <div style={{ fontSize: 16 }}>データを読み込んでいます...</div>
          </div>
        )}
                {!loading && tab === "settings" && (gender === "combined"
          ? <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 10, padding: 20, textAlign: "center", color: C.muted, fontSize: 15 }}>⚙️ 設定は「👨男子」「👩女子」を選んでから変更してください</div>
        ...
        {!loading && tab === "input" && (gender === "combined"
          ? <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 10, padding: 20, textAlign: "center", color: C.muted, fontSize: 15 }}>📝 入力は「👨男子」「👩女子」を選んでから行ってください</div>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "0 16px 40px" }}>
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 8, padding: "10px 14px", fontSize: 13, color: C.muted, lineHeight: 1.8 }}>
  ...
  return (
    <div>
      <div style={{ marginBottom: 12 }}>
        <button onClick={() => setSettingsOpen(v => !v)} style={{ width: "100%", background: settingsOpen?"#0d1e3a":C.surface, border: `1px solid ${settingsOpen?C.slalom:C.border}`, borderRadius: settingsOpen?"10px 10px 0 0":10, color: settingsOpen?C.slalom:C.muted, fontSize: 14, fontWeight: 700, padding: "10px 14px", cursor: "pointer", display: "flex", alignItems: "center", gap: 8 }}>
          <span>⚙️ 大会URL設定</span>
          {compUrl && !settingsOpen && <span style={{ fontSize: 12, background: "#1a3060", color: C.slalom, padding: "2px 8px", borderRadius: 10, marginLeft: "auto" }}>設定済み</span>}
          <span style={{ marginLeft: "auto", fontSize: 12 }}>{settingsOpen?"▲":"▼"}</span>
        </button>
        {settingsOpen && (
          <div style={{ background: C.surface, border: `1px solid #2a3a5a`, borderTop: "none", borderRadius: "0 0 10px 10px", padding: 14 }}>
            <div style={{ marginBottom: 12 }}>
              <div style={{ fontSize: 13, color: "#4a6a9a", marginBottom: 6 }}>ライブスコア／大会ページのURL</div>
              <input type="text" value={inputUrl} onChange={e => setInputUrl(e.target.value)} placeholder="https://ems.iwwf.sport/Competitions/Details?Id=..." style={{ width: "100%", background: "#0a1020", border: "1px solid #1e2a4a", borderRadius: 6, color: C.text, fontSize: 14, padding: "8px 10px", outline: "none", fontFamily: "monospace", boxSizing: "border-box" }} />
              <div style={{ marginTop: 6, fontSize: 12, color: "#4a6a9a" }}>IWWF・EMSどちらのページのURLでも貼り付けられます</div>
            </div>
            <div style={{ marginBottom: 14 }}>
              <div style={{ fontSize: 13, color: "#4a6a9a", marginBottom: 6 }}>大会名（表示用）</div>
              <input type="text" value={inputName} onChange={e => setInputName(e.target.value)} placeholder="例：インカレ2026" style={{ width: "100%", background: "#0a1020", border: "1px solid #1e2a4a", borderRadius: 6, color: C.text, fontSize: 15, padding: "8px 10px", outline: "none", boxSizing: "border-box" }} />
            </div>
            <button onClick={handleSave} disabled={saving} style={{ width: "100%", background: saved?C.positive+"22":"#1a3a6a", border: `1px solid ${saved?C.positive:C.slalom}`, borderRadius: 8, color: saved?C.positive:C.slalom, fontSize: 15, fontWeight: 700, padding: "11px", cursor: "pointer" }}>
              {saved?"✓ 保存しました":saving?"保存中...":"保存する"}
            </button>
          </div>
        )}
      </div>
      <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 12px", background: "#0a1020", borderRadius: 8, marginBottom: 12, fontSize: 13, color: compUrl?"#5abf8a":"#4a5580" }}>
      ...
      <div style={{ background: C.surface, border: "1px solid #1e3a6e", borderRadius: 12, overflow: "hidden", marginBottom: 12 }}>
        <div style={{ background: "#0d2045", padding: "9px 14px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13, fontWeight: 700, color: C.slalom }}>
            <span style={{ width: 7, height: 7, borderRadius: "50%", background: compUrl?"#ff4444":"#5a5a6a", display: "inline-block" }} />
            ライブスコア／大会結果
          </div>
          {compUrl && <a href={compUrl} target="_blank" rel="noopener noreferrer" style={{ fontSize: 12, color: "#4a6a9a", textDecoration: "none", border: "1px solid #1e3060", borderRadius: 4, padding: "2px 7px" }}>別窓 ↗</a>}
        </div>
        {compUrl ? (
          ...
          <div style={{ padding: "32px 16px", textAlign: "center" }}>
            <div style={{ fontSize: 29, marginBottom: 10 }}>🏁</div>
            <div style={{ fontSize: 14, color: "#3a4a6a", lineHeight: 1.6 }}>大会のURLを設定すると<br />ここに表示されます</div>
          </div>
        )}
      </div>
      <a href="https://twitter.com/JCWFgakuren" target="_blank" rel="noopener noreferrer" style={{ textDecoration: "none", display: "block", marginBottom: 12 }}>
        <div style={{ background: C.surface, border: "1px solid #1e2a4a", borderRadius: 12, overflow: "hidden" }}>
          <div style={{ background: "#0a1a30", padding: "9px 14px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: C.slalom, display: "flex", alignItems: "center", gap: 6 }}>
              <span style={{ fontSize: 15, fontWeight: 900, color: "#fff" }}>𝕏</span>@JCWFgakuren 速報
            </div>
            <span style={{ fontSize: 12, color: "#4a6a9a", border: "1px solid #1e2a4a", borderRadius: 4, padding: "2px 7px" }}>開く ↗</span>
          </div>
          <div style={{ background: "#000", padding: "20px 16px", display: "flex", alignItems: "center", gap: 12 }}>
            <span style={{ fontSize: 29, fontWeight: 900, color: "#fff" }}>𝕏</span>
            <div>
              <div style={{ fontSize: 15, color: "#7eb8f7", fontWeight: 700, marginBottom: 4 }}>@JCWFgakuren のポストを見る</div>
              <div style={{ fontSize: 13, color: "#4a6a9a" }}>全日本学生水上スキー連盟 — 大会速報はこちら</div>
            </div>
          </div>
        </div>
      </a>
